    ...
    with sqlite3.connect('movies.db') as db:
        ...
        for movie in movies:
            ...
            actors_data = actors_cursor.execute("""
                ...
                WHERE ma.movie_id = ?
            """, (movie_id,)).fetchall()
            actor_names = ", ".join([a[0] for a in actors_data])
...
def add_movie(movie: Movie):
    with sqlite3.connect('movies.db') as db:
        cursor = db.cursor()
        cursor.execute("INSERT INTO movies (title, year, director_id) VALUES (?, ?, ?)", 
        ...
        db.commit()
    return {"id": new_movie_id, "title": movie.title, "year": movie.year}
